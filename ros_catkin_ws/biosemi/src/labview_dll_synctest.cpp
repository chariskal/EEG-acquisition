...
//	DWORD	seam;
	...
	for (int nbuffers=0;;nbuffers++)
		...
		if (nbuffers%printFreq == 0)
			...
			float bytesPerMsec = BSIF_GET_BYTES_PER_MSEC();
			int throughPut = 0;
			if ((bytesPerMsec > 1.) && (nch > 0))
			throughPut = (int)(bytesPerMsec*1000./(double)(nch*4));
   			
			dsec = difftime(curTime, startTime);
			printf("at second %d, buffer %d, #bytes %d, seam %d, word throughput/channel/sec %d\n",
				(int)dsec, nbuffers, (int)bytesRead, seam, throughPut);
			nStatusLines++;		
